...
def input_fn_raw(base, dset, min_for_known, batchsize):
    ...
    data = data.map(
        lambda ind, label: tuple(
            tf.py_func(to_inds, [ind, label], [tf.int32, tf.int32, tf.int32], False)))
    data = data.padded_batch(batchsize, padded_shapes=((-1,), (), (1,)))

    def pack_in_dict(seqs, lengths, labels):
        return {"seq": seqs, "length": lengths}, labels
